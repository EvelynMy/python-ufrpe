    ...
    for festa in range(f):
        ...
        while True:
            ...
            if entrada[0] == '-1':
                ...
                for j in entrada:
                    mesa.colocar_na_fila(j)
                cnt += 1
            else:
                jogadores[str(cnt)] = Fila()
                for j in entrada:
                    jogadores[str(cnt)].colocar_na_fila(j)
                cnt += 1
        ...
        vencedor = False
        while rodada <= 1000:
            carta_mesa = mesa.tirar_da_fila()
            #print(carta_mesa)
            for num, jogador in jogadores.items():
                if jogador.lista_esta_vazia():
                    print(num)
                    rodada = 1000
                    vencedor = True
                else:
                    carta_jogador = jogador.tirar_da_fila()
                    if carta_jogador != carta_mesa:
                        jogador.colocar_na_fila(carta_jogador)
                    
            mesa.colocar_na_fila(carta_mesa)
            rodada += 1   
